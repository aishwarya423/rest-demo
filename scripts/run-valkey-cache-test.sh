#!/usr/bin/env bash
# =============================================================================
# RUN + TEST entity caching on VALKEY — fully in Docker, one command.
#
#   scripts/run-valkey-cache-test.sh
#
# Steps: clean slate -> start stack on Valkey -> wait for gateway -> flush cache
...
#
# Valkey is a drop-in Redis fork: the ONLY differences vs the redis script are
# the image (valkey/valkey) and the CLI (valkey-cli). The gateway config is
# identical (redis:// URL). Output file: valkey-cache-test-report.txt
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")/.."

# ---- backend (the ONLY difference vs the redis script) ---------------------
LABEL="VALKEY"
...
REPORT="valkey-cache-test-report.txt"
